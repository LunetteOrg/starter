...
- **Stories are tests**: `@storybook/addon-vitest` runs every story in a real browser as part of `pnpm test` ([ADR-0004 — component tests](./0004-testing.md#component-tests)). A story with a `play` function also asserts behaviour, so the showcase and the component suite are the same artefact.
- **A11y**: `@storybook/addon-a11y` with `a11y.test: 'error'` — an axe violation **fails the build**. This is what makes "a component is not done without an a11y pass" enforceable; drop to `'todo'` to report without blocking.
- `reactDocgen: 'react-docgen'` documents component props automatically. Not the `react-docgen-typescript` variant: it drives docgen through the TypeScript compiler API, which the native TS 7 compiler no longer exposes, crashing the build.
...
- − One extra heavy dependency (Storybook + Vite) in the monorepo — no longer for dev/docs alone, since it also carries the component-test layer ([ADR-0004 — component tests](./0004-testing.md#component-tests)), which is what makes the weight worth carrying.
